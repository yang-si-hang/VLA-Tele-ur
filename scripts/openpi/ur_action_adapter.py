...
class RTCActionChunkBroker(_base_policy.BasePolicy):
    ...
    def infer(self, obs: dict) -> dict:
        """
        当前代码中, RTC 严格按照 replan_interval 步长进行推理 (第二个 Chunk 在 replan_interval+1 步开始推理)
        """
        ...
        if self._pending_future is None and self._cur_step >= self._replan_interval:
            self._start_async_inference(obs)    # 传递当前步的 obs
    ...
    def _install_pending_chunk(self, *, block: bool) -> None:
        ...
        self._validate_chunk(result)
        elapsed_steps = self._cur_step - self._request_cursor   # 此时 _cur_step 还没有执行
        ...
        self._cur_step = elapsed_steps      # 要执行的 action 在 New chunk 的索引
