...
Camera acquisition, teleoperation control, dataset sampling, and optional UR
RTDE file recording use independent frequencies. Robotiq uses its driver
default. Each dataset frame pairs one control observation with the action
successfully sent from that same control step. A fixed-rate sampling thread
adds the latest camera frames, and a separate writer thread commits queued
samples to the dataset. Each saved episode optionally has a raw RTDE CSV under
the dataset's ``rtde`` directory containing only ``timestamp``,
``actual_TCP_pose``, and ``actual_TCP_speed``.

The two Orbbec cameras require a Sync Hub connection. They are configured as
primary and secondary, synchronized once before camera warm-up, and remain
physically connected with the robot and Sigma for the whole session. Dataset
recording and Sigma-to-UR coupling are independent states:
...
Example:
    python scripts/record_sigma_ur10e_sync.py --camera-fps 60 --control-fps 60 --dataset-fps 20 --record-rtde-trajectory --rtde-frequency 500
"""
...
WRIST_CAMERA_SERIAL = "CP9JA5300068"     # set to primary
BASE_CAMERA_SERIAL = "CP9JA530008V"    # set to secondary
DEFAULT_CAMERA_FPS = 60
DEFAULT_CONTROL_FPS = 500
DEFAULT_DATASET_FPS = 30
DEFAULT_RTDE_FREQUENCY = 500.0
DEFAULT_FREQUENCY_WARNING_RATIO = 0.95
DEFAULT_NUM_EPISODES = 10
DEFAULT_EPISODE_TIME_S = 90.0
DEFAULT_REFERENCE_SAMPLES = 20      # number of Sigma-UR teleoperation 0 reference point.cdcdcsnsrsnsrsnsnsrsnq
DEFAULT_POSITION_SCALE = 1.
DEFAULT_BASE_ROTATION_RAD = (0.0, 0.0, math.pi/2)
...
RTDE_RECORDING_VARIABLES = ("timestamp", "actual_TCP_pose", "actual_TCP_speed")


class SynchronizedCameraAugmentedURRobot(CameraAugmentedURRobot):
    """Connect two hardware-synchronized Orbbec cameras before the UR robot."""

    def __init__(self, config: URRobotConfig, camera_configs: dict[str, CameraConfig], sdk_context: object) -> None:
        cameras = {name: OrbbecCamera(camera_config, sdk_context=sdk_context) for name, camera_config in camera_configs.items()}
        super().__init__(config, camera_configs, cameras=cameras)
    ...
    def connect(self, calibrate: bool = True) -> None:
        connected_cameras = []
        try:
            ordered_cameras = sorted(self.cameras.values(), key=lambda camera: {"secondary": 0, "primary": 1}.get(camera.sync_mode, 2))
            for camera in ordered_cameras:
                camera.connect(warmup=False, synchronize_clock=False)
                connected_cameras.append(camera)
            self.sdk_context.enable_multi_device_sync(0)
            for camera in connected_cameras:
                camera.warmup()
            URRobot.connect(self, calibrate=calibrate)
            for camera in connected_cameras:
                camera.start_frame_rate_monitoring()
        except BaseException:
            for camera in connected_cameras:
                with contextlib.suppress(BaseException):
                    camera.stop_frame_rate_monitoring()
            if URRobot.is_connected.fget(self):
                with contextlib.suppress(BaseException):
                    URRobot.disconnect(self)
            for camera in reversed(connected_cameras):
                if camera.is_connected:
                    with contextlib.suppress(BaseException):
                        camera.disconnect()
            raise
        logger.info("UR robot and %d synchronized cameras connected", len(self.cameras))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--root", type=Path, default=DATA_PATH / f"plug_{timestamp}")
    ...
    parser.add_argument("--record-rtde-trajectory", default=True, action=argparse.BooleanOptionalAction, help="Record one raw UR RTDE CSV for each saved episode")
    ...
    parser.add_argument("--primary-camera-serial", dest="primary_camera_serial", default=WRIST_CAMERA_SERIAL)
    parser.add_argument("--secondary-camera-serial", dest="secondary_camera_serial", default=BASE_CAMERA_SERIAL)
    parser.add_argument("--primary-camera-resolution", dest="primary_camera_resolution", type=int, nargs=2, metavar=("WIDTH", "HEIGHT"), default=(320, 240))
    parser.add_argument("--secondary-camera-resolution", dest="secondary_camera_resolution", type=int, nargs=2, metavar=("WIDTH", "HEIGHT"), default=(320, 240))
    ...
    for camera_name in ("primary_camera", "secondary_camera"):
...
def run_continuous_session(
    *,
    ...
    rtde_receiver: rtde_receive.RTDEReceiveInterface | None = None,
    rtde_output_dir: Path | None = None,
    ...
    if (rtde_receiver is None) != (rtde_output_dir is None):
        raise ValueError("RTDE receiver and output directory must be provided together")
    ...
    active_rtde_path: Path | None = None
    pending_rtde_path: Path | None = None
    ...
    def confirm_task_selection(selected_index: int) -> None:
        ...
        if rtde_receiver is not None and rtde_output_dir is not None:
            rtde_output_dir.mkdir(parents=True, exist_ok=True)
            active_rtde_path = rtde_output_dir / f".episode_{episode_generation:06d}.csv.tmp"
            if not rtde_receiver.startFileRecording(str(active_rtde_path), list(RTDE_RECORDING_VARIABLES)):
                active_rtde_path = None
                raise RuntimeError("Failed to start UR RTDE file recording")
    ...
    def enqueue_boundary(*, save: bool) -> None:
        nonlocal episode_state, active_rtde_path, pending_rtde_path
        if rtde_receiver is not None and active_rtde_path is not None:
            if not rtde_receiver.stopFileRecording():
                raise RuntimeError("Failed to stop UR RTDE file recording")
            pending_rtde_path = active_rtde_path
            active_rtde_path = None
    ...
    def process_writer_results() -> None:
        ...
        while True:
            ...
            if result.saved:
                if pending_rtde_path is not None and rtde_output_dir is not None:
                    final_rtde_path = rtde_output_dir / f"episode_{recorded_episodes:06d}.csv"
                    pending_rtde_path.replace(final_rtde_path)
                    pending_rtde_path = None
                recorded_episodes += 1
                log_say(
                ...
                if pending_rtde_path is not None:
                    pending_rtde_path.unlink(missing_ok=True)
                    pending_rtde_path = None
                logger.warning("Empty episode was not saved")
    ...
    try:
            ...
            for command in _get_pending_commands(command_queue):
                ...
                if command is SessionCommand.ENABLE_TELEOP:
                    if episode_state is EpisodeState.RECORDING:
                        ...
                        logger.info("Teleoperation is already enabled")
                    else:
                        capture_current_references(
                ...
                if command is SessionCommand.DISABLE_TELEOP:
                    ...
                    if teleop_state is TeleoperationState.ENABLED:
                        ...
                        logger.info("Teleoperation is already disabled")
                    continue
                ...
                if command is SessionCommand.START_EPISODE:
                    if episode_state is EpisodeState.FINISHED:
                        ...
                        logger.warning("The previous episode is still being saved")
                    elif episode_state is EpisodeState.SELECTING_TASK:
                        logger.info("Task selection is already in progress")
                    elif episode_state is EpisodeState.RECORDING:
                        logger.info("An episode is already being recorded")
                        ...
                        confirm_task_selection(3)   # default task 3
                    continue
                ...
                if command is SessionCommand.FINISH_EPISODE:
                    if episode_state is EpisodeState.RECORDING:
                        ...
                        logger.info("No episode is currently recording")
                    continue
        ...
        if rtde_receiver is not None and active_rtde_path is not None:
            ...
            active_rtde_path.unlink(missing_ok=True)
            active_rtde_path = None
        stop_sampler()
        stop_writer()
        process_writer_results()
...
def run(args: argparse.Namespace) -> LeRobotDataset:
    ...
    primary_width, primary_height = args.primary_camera_resolution
    secondary_width, secondary_height = args.secondary_camera_resolution
    camera_configs: dict[str, CameraConfig] = {
        "left_wrist_0_rgb": OrbbecCameraConfig(
            serial_number_or_name=args.primary_camera_serial,
            width=primary_width,
        ...
        "base_0_rgb": OrbbecCameraConfig(
            serial_number_or_name=args.secondary_camera_serial,
    ...
    sdk_context = ob.Context()
    robot = SynchronizedCameraAugmentedURRobot(
        URRobotConfig(
    ...
    try:
        ...
        robot.connect()
        teleop.connect()
        if args.record_rtde_trajectory:
            rtde_receiver = rtde_receive.RTDEReceiveInterface(args.robot_ip, DEFAULT_RTDE_FREQUENCY, list(RTDE_RECORDING_VARIABLES))
        ...
        run_continuous_session(
            ...
            rtde_output_dir=args.root / "rtde" if rtde_receiver is not None else None,
        )
    except BaseException:
        failed = True
        ...
        dataset.finalize()
        if failed:
            logger.error("Recording stopped because of an error")
            log_say(
            ...
            log_say(
                f"Recording finished. Data saved to {dataset.root.parts[-2:]}",
                blocking=True,
            )
    return dataset
